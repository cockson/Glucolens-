...
def main():
    ...
    try:
        ...
        for o in outcomes:
            ...
            qskin = db.query(PredictionRecord).filter(
                PredictionRecord.org_id == rec.org_id,
                PredictionRecord.modality == "skin",
            ...
            if qskin:
                try:
                    s = json.loads(qskin.output_json or "{}")
                except Exception:
                    s = {}
            ...
            rows.append({
                ...
                "p_retina": (float(p_ret) if p_ret is not None else None),
